...
const YEAR = 2023;  // Cambiar a 2023, 2025, etc. para otros dashboards
...
function analyzeQuotaMensual(filteredIngresos) {
    ...
    let regular = { cantidad: 0, total: 0 };      // para 43500 O 35000
    let damaEstudiante = { cantidad: 0, total: 0 }; // para 21750 O 20000
    let pasivo = { cantidad: 0, total: 0 };         // para 13050 O 10500
    let otros = { cantidad: 0, total: 0 };
    
    cuotaMensualItems.forEach(item => {
        const monto = item.importe;
        // REGULAR: 43500 (después del alza) o 35000 (antes del alza)
        if (monto === 43500 || monto === 35000) {
            regular.cantidad++;
            regular.total += monto;
        }
        // DAMA / ESTUDIANTE: 21750 (2024) o 20000 (2023)
        else if (monto === 21750 || monto === 20000) {
            damaEstudiante.cantidad++;
            damaEstudiante.total += monto;
        }
        // PASIVO: 13050 (2024) o 10500 (2023)
        else if (monto === 13050 || monto === 10500) {
            pasivo.cantidad++;
            pasivo.total += monto;
        }
        else {
    ...
    const quotaData = [];
    if (regular.cantidad > 0) quotaData.push({ 
        tipo: "REGULAR", 
        montoUnitario: "$43.500 / $35.000", 
        cantidad: regular.cantidad, 
        total: regular.total 
    });
    if (damaEstudiante.cantidad > 0) quotaData.push({ 
        tipo: "DAMA / ESTUDIANTE", 
        montoUnitario: "$21.750 / $20.000", 
        cantidad: damaEstudiante.cantidad, 
        total: damaEstudiante.total 
    });
    if (pasivo.cantidad > 0) quotaData.push({ 
        tipo: "PASIVO", 
        montoUnitario: "$13.050 / $10.500", 
        cantidad: pasivo.cantidad, 
        total: pasivo.total 
    });
    if (otros.cantidad > 0) quotaData.push({ 
        tipo: "📦 OTROS", 
        montoUnitario: "---", 
        cantidad: otros.cantidad, 
        total: otros.total 
    });
...
function getRegularEvolution() {
    ...
    ingresosData.forEach(ing => {
        ...
        // Incluir AMBOS montos: 43500 (después del alza) y 35000 (antes del alza)
        if (categoria === "CUOTA MENSUAL" && (ing.importe === 43500 || ing.importe === 35000) && ing.mes >= 1 && ing.mes <= 12) {
